import {
  defineView, registerRoute, ref, computed, watch, onUnmounted, store, wsSend, toast, haptic,
} from "../ui.js";
...
registerRoute("/games/board", defineView("gameBoard", {
  ...
  <div class="page-plain">
    ...
    <div class="row gap3 mt5">
      <button v-if="canStart" class="btn btn-primary grow" @click="start">开始对局</button>
    ...
    <p v-if="aborted" class="sub center mt4">对局已中止，房间还在——等新同学加入就能重新开始</p>

    <Transition name="mat">
      <div v-if="finished" class="overlay glass glass-thick">
        <h2 class="t2">{{ resultTitle }}</h2>
        <p class="sub mt2">{{ resultReason }}</p>
        <div class="row gap3 mt5">
          <button class="btn grow" @click="leaveRoom(false)">离开房间</button>
          <button class="btn grow" :class="othersWantRematch ? 'btn-green' : 'btn-primary'" @click="rematch">
  ...
  setup() {
    ...
    const statusText = computed(() => {
      ...
      if (!roomApi.room.value.started) return players.value.length < 2 ? "等待对手加入" : "可以开始了";
    ...
    return { ...roomApi, board, size, last, players, spectators, finished, myMark, isMyTurn, canStart,
             statusText, resultTitle, resultReason, markOf, isTurn, play, start };
  },
}));
