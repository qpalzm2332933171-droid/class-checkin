...
/* 匿名开关是"全局"的：在讨论主界面选一次，进任何话题发言都跟着走。
   默认实名，状态记在本地，重开 App 也保持。 */
const ANON_KEY = "chat_anon_mode";
const anonMode = ref(localStorage.getItem(ANON_KEY) === "1");
function setAnonMode(on) {
  anonMode.value = !!on;
  try { localStorage.setItem(ANON_KEY, on ? "1" : "0"); } catch (err) { /* 隐私模式忽略 */ }
  haptic(6);
}
...
const SHARED_STYLE = `
  ...
  .anon-pill { display: inline-flex; align-items: center; gap: 5px; border: 1px solid var(--hair);
    background: var(--mat-thin); color: var(--ink-3); font-size: var(--fs-sub); font-weight: 700;
  ...
  .anon-pill:active { transform: scale(0.94); }
  .anon-pill.on { color: #fff; border-color: transparent;
    background: linear-gradient(180deg, color-mix(in srgb, var(--orange) 92%, #fff 8%), var(--orange));
    box-shadow: 0 6px 16px color-mix(in srgb, var(--orange) 30%, transparent); }
  .anon-tip { color: var(--ink-3); margin-top: 6px; }
...
registerRoute("/chat", defineView("chatTopics", {
    ...
    <header class="big-title row-between">
      ...
      <div class="row gap2">
        <button class="anon-pill" :class="{ on: anonMode }" @click="setAnonMode(!anonMode)">
          <Icon :n="anonMode ? 'eyeOff' : 'eye'" :size="17" />
          <span>{{ anonMode ? '匿名' : '实名' }}</span>
        </button>
        <button class="btn btn-icon glass glass-thin" @click="refresh"><Icon n="refresh" :size="19" /></button>
      </div>
    </header>

    <p class="cap anon-tip">
      {{ anonMode ? '匿名模式：接下来在任何话题里发言都是匿名的' : '实名模式：发言会显示你的名字和头像，点右上角可切换匿名' }}
    </p>
  ...
  setup() {
    ...
    const form = ref({ title: "", content: "", anon: anonMode.value });
    const myName = computed(() => (store.user && store.user.name) || "我");
    let stopNew = null;
    let stopBump = null;
    ...
    function compose() { form.value = { title: "", content: "", anon: anonMode.value }; sheet.value = true; }
...
registerRoute("/chat/topic", defineView("chatTopic", {
  ...
  setup() {
    ...
    const messages = ref([]);
    const draft = ref("");
    const anon = anonMode;               // 与讨论主界面共用同一个开关
