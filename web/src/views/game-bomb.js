...
registerRoute("/games/bomb", defineView("gameBomb", {
  ...
  <div class="page-plain">
    ...
    <div class="players mt5">
      <span v-for="p in players" :key="p.uid" class="chip chip-av" :class="chipOf(p)">
        ...
        {{ p.name }}{{ state.turn === p.uid ? ' · 该你了' : '' }}{{ isReady(p) && !finished ? ' · 已准备' : '' }}
      </span>
    </div>
    <p v-if="isSpectator" class="chip chip-orange mt4">观战模式</p>

    <div v-if="canReady" class="mt5">
      <button class="btn btn-block btn-lg" :class="myReady ? '' : 'btn-primary'" @click="toggleReady">
        {{ myReady ? '取消准备' : '准备' }}
      </button>
      <p class="cap center mt3">已准备 {{ ready.length }}/{{ players.length }} · 全部准备好自动开始（至少 2 人）</p>
    </div>

    <div v-else-if="!state.alive || !state.alive.length" class="mt5 center">
      <p class="cap">至少 2 人准备后才能开始，喊上同学一起来吧</p>
    </div>
    ...
    <div class="glass glass-thin list">
      <div v-for="(h, i) in history.slice().reverse()" :key="i" class="list-row">
        <span class="avatar avatar-sm">
          <img v-if="h.avatar" :src="mediaUrl(h.avatar)" :alt="h.name" loading="lazy" />
          <template v-else>{{ h.name.slice(0,1) }}</template>
        </span>
        <span class="grow elide">{{ h.name }}</span>
        <b class="num">{{ h.n }}</b>
        <span class="chip" :class="h.dir === 'low' ? 'chip-accent' : 'chip-orange'">{{ h.dir === 'low' ? '偏小' : '偏大' }}</span>
      </div>
    ...
    <Transition name="mat">
      <div v-if="finished || aborted" class="overlay glass glass-thick">
        <h2 class="t2">{{ aborted ? '本局已中止' : (isSpectator ? '本局结束' : (winners.includes(store.user?.id) ? '你活到了最后 🎉' : '炸了')) }}</h2>
        <p class="sub mt2">{{ aborted ? (reason || '有人离开了房间') : ('炸弹是 ' + bomb) }}</p>
  ...
  setup() {
    ...
    return { ...roomApi, state, history, guessValue, players, finished, winners, lo, hi, bomb, alive, isSpectator,
             isMyTurn, canStart, quick, statusText, pct, chipOf, submit, start, store, mediaUrl };
  },
}));
