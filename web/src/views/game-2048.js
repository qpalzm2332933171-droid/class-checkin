import { defineView, registerRoute, ref, computed, onMounted, onUnmounted, navigate, toast, haptic, confirmDialog, api, store } from "../ui.js";
...
registerRoute("/games/2048", defineView("game2048", {
  ...
  setup() {
    ...
    /* 合成 2048 的积分上报（服务端按 kind 记 1 分） */
    async function award2048(count) {
      toast("🎉 合成 2048，积分 +" + count, "ok", 3600);
      haptic([10, 40, 10, 40, 20]);
      for (let i = 0; i < count; i++) {
        try {
          const res = await api("/api/games/solo/points", { method: "POST", body: { kind: "2048" } });
    ...
    function move(dir) {
      ...
      setTimeout(() => {
        ...
        for (const slot of merges) {
          ...
          if (slot.keep.v === 2048) made2048++;   // 每合成出一个 2048 记 1 分
        }
        if (made2048) award2048(made2048);
        if (merges.length) {
